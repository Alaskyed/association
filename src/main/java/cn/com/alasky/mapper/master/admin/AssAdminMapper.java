...
public interface AssAdminMapper {
     ...
     * @return
     */
    @Select("SELECT a.ass_uuid AS ass_uuid, un.university_name AS ass_university, a.association_name AS ass_name, " +
            "COUNT(*) AS ass_member_count, a.custom_url AS custom_url\n" +
     ...
     */
    @Update("UPDATE association_info " +
            "SET custom_url = #{newData} " +
            "WHERE ass_uuid=#{assUuid}")
    int updateAssCustomUrl(String assUuid, String newData);
     ...
     * @param assUuid
     */
    @Delete("delete from stu_ass where ass_uuid = #{assUuid} ")
    void deleteMemberByAssUuid(String assUuid);

    /**
     * 根据社团名称删除社团里所有的部门
     * @param assUuid
     */
    @Delete("delete from departments where ass_uuid = #{assUuid} ")
    void deleteDepByAssUuid(String assUuid);
     ...
     * @param assUuid
     */
    @Delete("delete from association_info where ass_uuid = #{assUuid} ")
    void deleteAssByAssUuid(String assUuid);
}
